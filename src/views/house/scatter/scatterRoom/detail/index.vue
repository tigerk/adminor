<template>
  <div class="scatter-room-detail-page pf-page">
    <el-page-header class="scatter-room-detail-page__head" @back="goBack">
      <template #content>
        <div class="scatter-room-detail-page__meta">
          <div class="scatter-room-detail-page__title-row">
            <div class="scatter-room-detail-page__title">{{ detail?.houseName || "房源详情" }}</div>
            <el-tag v-if="currentRoom?.roomNumber" effect="plain">{{ currentRoom.roomNumber }}</el-tag>
          </div>
          <div class="scatter-room-detail-page__desc">查看房源、房间、租客、预定和跟进记录。</div>
        </div>
      </template>
      <template #extra>
    ...
    <section class="scatter-room-detail-page__body">
      <HouseViewDialog
        :loading="loading"
        :detail="detail"
...
<script setup lang="ts">
  ...
  defineOptions({ name: "ScatterRoomDetail" });
