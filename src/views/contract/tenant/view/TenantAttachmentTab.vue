<template>
  <div class="tenant-attachment-tab">
    <section class="attachment-section">
      <div class="section-head">
        <div>
          <div class="section-title">资料附件</div>
          <div class="section-desc">按资料用途分组管理租约相关附件；线下合同、补充协议和租客资料可统一归档在这里。</div>
        </div>
...
<script setup lang="ts">
  ...
  import { updateLeaseAttachments } from "@/api/contract/tenant";
  ...
  const categoryOptions = [
    ...
    { value: "AUTHORIZATION", label: "授权委托书", desc: "租客授权、代办手续等授权文件。" },
    { value: "TENANT_MATERIAL", label: "租客资料", desc: "租客补充证件、承诺书、证明材料等。" },
  ...
  const leaseId = computed(() => props.detailData?.leaseId);
  const attachmentReadonly = computed(() => props.readonly || props.detailData?.status === LeaseStatusEnumMeta.VOIDED.code);
  ...
  watch(
    () => props.detailData?.leaseAttachmentGroupList,
    groups => {
  ...
  async function saveAttachments() {
    ...
    const resp = await updateLeaseAttachments({
