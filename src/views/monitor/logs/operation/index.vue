  ...
  <div class="main">
    <el-form ref="formRef" :inline="true" :model="form" class="search-form bg-bg_color w-full pl-8 pt-[12px] overflow-auto">
      <el-form-item label="所属模块" prop="title">
        <el-input v-model="form.title" placeholder="请输入所属模块" clearable class="w-[170px]!" />
      </el-form-item>
      ...
      <template v-slot="{ size, dynamicColumns }">
        ...
        >
          <template #operation="{ row }">
            <el-button class="reset-margin outline-hidden!" link type="primary" :size="size" :icon="useRenderIcon(View)" @click="onDetail(row)">详情</el-button>
          </template>
        </pure-table>
      </template>
    </PureTableBar>
