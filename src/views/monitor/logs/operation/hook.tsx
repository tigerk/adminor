...
export function useRole(tableRef: Ref) {
  ...
  function onBatchDel() {
    ...
    batchDeleteOperationLogs(getKeyList(curSelected, "id")).then(() => {
  ...
  function onDetail(row: any) {
    getOperationLogDetail({ id: row.id }).then(res => {
      addDialog({
        title: "系统日志详情",
        fullscreen: true,
        hideFooter: true,
  ...
  async function onSearch() {
    ...
    const { data } = await getOperationLogList(submitForm);
