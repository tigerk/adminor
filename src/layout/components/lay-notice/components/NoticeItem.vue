<script setup lang="ts">
  ...
  function handleItemClick() {
    if ((props.noticeItem?.type === "2" || props.noticeItem?.type === "1") && props.noticeItem?.isRead === false) {
      emit("item-click", props.noticeItem);
    }
  }
</script>

<template>
  <div class="notice-container border-0 border-b-[1px] border-solid border-[#f0f0f0] dark:border-[#303030]" @click="handleItemClick">
    <el-avatar v-if="noticeItem.avatar" :size="30" :src="noticeItem.avatar" class="notice-container-avatar" />
    <div class="notice-container-text">
      <div class="notice-text-title text-[#000000d9] dark:text-white">
        <el-tag v-if="noticeItem?.type === '2' || noticeItem?.type === '1'" :type="noticeItem?.isRead === false ? 'danger' : 'info'" size="small" class="notice-title-unread">
          {{ noticeItem?.isRead === false ? "未读" : "已读" }}
        </el-tag>
        <el-tag v-if="noticeItem?.type === '3' && noticeItem?.priority != null" type="warning" size="small" class="notice-title-unread">
          {{ NOTICE_TODO_PRIORITY_HELPER.getNameByCode(noticeItem?.priority) }}
        </el-tag>
