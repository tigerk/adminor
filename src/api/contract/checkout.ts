...
/** 获取退租初始化数据 */
export const getCheckoutInitData = (tenantId: number) => {
...
export const saveCheckout = (data: CheckoutFormProps) => {
  return http.request<ApiResponse<CheckoutInitDataProps>>("post", baseUrlApi("/tenant/checkout/save"), { data });
};

/** 提交退租审批 */
export const submitCheckout = (checkoutId: number) => {
...
/** 取消退租单 */
export const cancelCheckout = (checkoutId: number) => {
...
/** 获取退租单详情 */
export const getCheckoutDetail = (checkoutId: number) => {
...
/** 根据租客ID获取退租单 */
export const getCheckoutByTenantId = (tenantId: number) => {
