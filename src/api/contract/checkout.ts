...
export const getCheckoutInitData = (tenantId: string) => {
  return http.request<ApiResponse<CheckoutInitDataProps>>("post", baseUrlApi("tenant/checkout/init"), { data: { tenantId } });
};
...
export const submitCheckout = (checkoutId: string) => {
  return http.request<ApiResponse<void>>("post", baseUrlApi("tenant/checkout/submit"), { data: { tenantId: checkoutId } });
};

/** 取消退租单 */
export const cancelCheckout = (checkoutId: string) => {
  return http.request<ApiResponse<void>>("post", baseUrlApi("tenant/checkout/cancel"), { data: { tenantId: checkoutId } });
};

/** 获取退租单详情 */
export const getCheckoutDetail = (checkoutId: string) => {
  return http.request<ApiResponse<CheckoutDetailProps>>("post", baseUrlApi("tenant/checkout/detail"), { data: { tenantId: checkoutId } });
};

/** 根据租客ID获取退租单 */
export const getCheckoutByTenantId = (tenantId: string) => {
  return http.request<ApiResponse<CheckoutDetailProps>>("post", baseUrlApi("tenant/checkout/getByTenant"), { data: { tenantId } });
};
